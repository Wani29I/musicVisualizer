...
class Music_Genre_CNN(object):
    ...
    def new_song_spectrogram_prediction_2_str(self, song_mel_spectrogram):
        predict_str = ''
        
        minIndex = 0
        maxIndex = 256
        
        while( maxIndex < (song_mel_spectrogram.shape[1] - 256) ):        
            segment = []
            segment.append(song_mel_spectrogram[:, minIndex : maxIndex])
            segment_array = np.asarray(segment)[:, :, :, np.newaxis]
    ...
    def test_model(self, test_X, test_T, overlap=0.5):
        ...
        for iii in range(len(test_X)):
            ...
            # print(len(song_mel_spectrogram))
            predicted_labels[iii], _ = self.song_spectrogram_prediction(song_mel_spectrogram, overlap=overlap)
            # print( 'predicted:', predicted_labels[iii], 'correct:', correct_labels[iii])
    ...
    def test_ice_model(self, test_X, test_T, overlap=0.5):
        # test the accuracy of the model using testing data
        num_sample = len(test_T)
        predicted_labels = np.zeros(num_sample)
        for iii in range(len(test_X)):
    ...
    def test_model_new(self, test_X, test_T):
        # test the accuracy of the model using testing data
        ...
        for iii in range(len(test_X)):
            song_mel_spectrogram = test_X[iii]
            # print(song_mel_spectrogram.shape)
            predicted_labels = self.new_song_spectrogram_prediction_mid(song_mel_spectrogram.squeeze())
            print(correct_labels[iii], predicted_labels)
            
            for eachPredictedLabel in predicted_labels:
                PredictCount += 1
                if( eachPredictedLabel == correct_labels[iii] ):
                    correctPredictCount += 1
            # print( 'predicted:', predicted_labels[iii], 'correct:', correct_labels[iii])
            # correct_labels[iii] = np.argmax(test_T[iii])
            
        print("PredictCount = ", PredictCount )
        print("correctPredictCount = ", correctPredictCount )
        print("accuracy = ", correctPredictCount/PredictCount * 100, "%")